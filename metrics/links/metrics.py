...
from .Single_event import (
        links_precision_distance,
        links_recall_distance,
        links_precision_proba,
        links_recall_proba)
...
def pr_from_events(events_pred, events_gt, Trange):
    ...
    aff_partition = links_partition(events_pred, E_gt)

    d_precision = [links_precision_distance(Is, J) for Is, J in zip(aff_partition, events_gt)]

    d_recall = [links_recall_distance(Is, J) for Is, J in zip(aff_partition, events_gt)]


    p_precision = [links_precision_proba(Is, J, E) for Is, J, E in zip(aff_partition, events_gt, E_gt)]


    p_recall = [links_recall_proba(Is, J, E) for Is, J, E in zip(aff_partition, events_gt, E_gt)]
    ...
    dict_out = dict({'links_Precision': p_precision_average,
                     'links_Recall': p_recall_average,
